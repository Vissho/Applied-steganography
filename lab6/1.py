...
def main():
    image_path = input("\nВведите путь к изображению: ").strip().strip('"')
    text_message = input("Введите текстовое сообщение: ").strip()
    ...
    if not text_message:
        text_message = "hello"
        print(f"Используем сообщение по умолчанию: '{text_message}'")
    
    create_composite_audio(image_path, text_message, output_audio)
    ...
    print(f"\nИсходный текст: '{text_message}'")
    print(f"Извлеченный текст: '{extracted_text}'")
    
    if extracted_text == text_message:
        print("\nТекст извлечен корректно!")
    else:
        print("\nТекст извлечен с ошибками")
        print(f"  Ожидалось: '{text_message}'")
